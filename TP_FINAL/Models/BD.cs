...
public static class BD
{
    ...
    public static List<MODELO> ObtenerModelos(int pagMandada, int opcionSort = 1)
    {
        ...
        public static MODELO ObtenerModeloXId(int IdModelo)
    {
        MODELO unModelo;
        using (SqlConnection db = new SqlConnection(_connectionString))
        {
            string SP = "SP_ObtenerModeloXId";
            unModelo = db.QueryFirstOrDefault<MODELO>(SP, new { IdModelo = IdModelo }, commandType: CommandType.StoredProcedure);
            return unModelo;
        }
    }
            ...
            public static void AgregarABD(string Nombre, float Precio, string Descripcion, int FkGenero, int FkMarca, int Stock, string Foto)
    {
        using (SqlConnection db = new SqlConnection(_connectionString))
        {
            ...
            db.Execute(SP, new { Nombre = Nombre, Precio = Precio, Descripcion = Descripcion, FkGenero = FkGenero, FkMarca = FkMarca, Stock = Stock,Foto = Foto }, commandType: CommandType.StoredProcedure);
        }
    }
